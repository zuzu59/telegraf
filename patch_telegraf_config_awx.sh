#!/bin/bash
#Petit script pour copier et 'patcher' la config Telegraf dans un container Docker AWX sur Openshift sous Centos avec les bons sercrets qui se trouvent dans les variable d'environnements
#zf191010.0855, zf200629.1057, zf201110.1050

#BASH_XTRACEFD="5" && PS4='$LINENO: ' && set -e -v -x

zfile="/etc/telegraf/telegraf.conf"

sudo cp $(/usr/bin/dirname $0)/telegraf_awx.conf $zfile
